...
鸿蒙客户端默认消息端口为 `29000`、下载端口为 `25173`，端口输入留空时也使用这两个默认值。转发默认监听 `0.0.0.0`，手机应填写电脑实际 IP。桌面版可能自动更换后端端口，启动前需确认；转发脚本提供 `start`、`status`、`stop`，详情见 Skill。转发就绪不代表手机连通或后端业务正常。
